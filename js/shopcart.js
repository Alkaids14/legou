...
$(function() {
			...
			$tbodyInputs.change(function() { //给表格总单选框绑事件
				...
				$tbodyInputs.each(function(index, input) { //循环表格input
					...
					if (!checkState) { //如果有一个等于false
						flag = false; //标杆变为false(全选变为false)
					}
				})
				$theadInput.prop('checked', flag); //把状态赋值给头部全选框
				$totalPriceInput.prop('checked', flag); //把状态赋值给计算机全选框
				allTotal(); //总计
			})
			...
			*/
			$('.add').click(function() { //给增加绑定事件
				...
				count++; //自增
				$(this).next().val(count); //把自增赋值给后面输入框
				//小计
				subTotal($(this), count);
				allTotal(); //总计
			})
			...
			1给-绑定事件
			2获取前面输入框的值
			3值自减，边界判断，如果小于1，那么等于1 否则等于自己
			4把减少后的值赋值给前面的输入框*/
			$('.reduce').click(function() { //绑定事件
				var count = parseInt($(this).prev().val()); //获取前面输入框的值
				count--; //自减
				count = count < 1 ? 1 : count; //边界判断
				$(this).prev().val(count); //把减少后的值赋值给前面的输入框
				//小计
				subTotal($(this), count);
				allTotal(); //总计
			})
			/*
			封装一个小计函数：（点击+或-的时候，调用小计功能）
			*/
			function subTotal(dom, count) {
				var singlePrice = parseFloat(dom.closest('tr').find('.price').text()); //找到单价
				var subTotalPrice = singlePrice * count; //单价=小计*数量
				dom.closest('tr').find('.subprice').text(subTotalPrice.toFixed(2)); //把小计的结果渲染对应位置 保留2位小数
			}
			/*总计功能实现：（头部全选 计算总价全选	 表格的选择框+-删除 六个地方调用总计）
			1定义一个变量 用于保存总价 定义一个变量 保存已选商品数量
			2获取所有表格中的选择框 循环 获取选中状态 判断
			3如果选中，就要累加这一行的小计
			*/
			function allTotal() {
				var allPrice = 0; //定义一个变量 用于保存总价
				var selectedCount = 0; //定义一个变量 保存已选商品数量
